...
if (!apiKey) {
  ...
  const agentName = process.env.AGENT_NAME || "protoAgent";
  const agentTagline = process.env.AGENT_TAGLINE
    || "a LangGraph + A2A agent built on the protoAgent template";

  const SYSTEM_PROMPT = `\
You are writing release notes for ${agentName} — ${agentTagline}.

Given raw git commit subjects, rewrite them as polished release notes.

Rules:
- Group into 2–4 themed sections based on what actually shipped
...
const embedTitle = process.env.AGENT_NAME || "protoAgent";
const embedFooter = process.env.AGENT_TAGLINE
  ? `${embedTitle} — ${process.env.AGENT_TAGLINE}`
  : `${embedTitle} — protoLabs agent`;

const embed = {
  title: `${embedTitle} ${version}`,
  description: notes,
  color: 0x7c3aed,
